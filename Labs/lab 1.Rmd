...
rollLoadedDie <- function(rolls){
  ...
  final_vec = vector("double", rolls)

  rolls <- runif(rolls,0,1)
...
```{r}
## chisq to see if it is different from expected
...
trialSizes <- c(5, 10, 20, 25, 30, 40, 50, 100, 200, 300, 400, 500, 1000, 2000, 3000, 4000, 5000, 10000, 20000, 30000, 100000)
...
for( i in 1:length(trialSizes) ){
  rolls <- vector(mode = "double", length=length(trialSizes))
  
  means[i] <- mean(rollLoadedDie(i))
  variances[i] <- var(rollLoadedDie(i))
...
As we can see in the plot above, beginning at trial 200 the difference between the actual and expected mean is consistently below 0.5. Similarly, after trial 100, the difference between the actual and expected variance tends to be below 0.
